...
import com.spring.productsshop.service.ProductService;
...
public class ProductController {
    ...
    public ResponseEntity<ProductDTO> updateProduct(@PathVariable Long id, @RequestBody ProductDTO updatedProductDTO) {
        Product updatedProduct = ProductConvertTo.convertToEntity(updatedProductDTO);
 ...
 * "Product controller with CRUD Operations") public class ProductController {
 * 
 ...
 * "Products found, retrieved products", content = {
 * 
 ...
 * getAllProducts() { // Obtiene todos los productos de la base de datos en una
 * lista List<Product> products = productRepository.findAll(); // Si no hay
 * productos, lanza una excepción if (products.isEmpty()) { throw new
 * ResourceNotFoundException("No products found"); } // Devuelve los productos
 * en formato DTO return
 * ResponseEntity.ok(ProductConvertTo.convertToDTOList(products)); }
 * 
 ...
 * "Product not found with id: ...", content = @Content(schema
 * = @Schema(implementation = ErrorDetails.class))) })
 * 
 * @GetMapping("/products/{id}") public ResponseEntity<ProductDTO>
 * getProductById(@PathVariable Long id) { // Busca el producto por ID en la
 * base de datos Optional<Product> productOptional =
 * productRepository.findById(id); // Si no se encuentra el producto, lanza una
 * excepción Product product = productOptional .orElseThrow(() -> new
 * ResourceNotFoundException("Product not found with id: " + id)); // Devuelve
 * el producto encontrado en formato DTO return
 * ResponseEntity.ok(ProductConvertTo.convertToDTO(product)); }
 * 
 ...
 * "Invalid product data provided", content = @Content(schema
 * = @Schema(implementation = ErrorDetails.class))) })
 * 
 * @PostMapping("/products") public ResponseEntity<ProductDTO>
 * createProduct(@RequestBody ProductDTO productDTO) { // Si los datos del
 * producto son nulos, lanza una excepción de validación if (productDTO == null)
 * { throw new ValidationException("Invalid product data provided"); } //
 * Convierte el DTO del producto en una entidad de producto Product product =
 * ProductConvertTo.convertToEntity(productDTO); // Guarda el nuevo producto en
 * la base de datos Product savedProduct = productRepository.save(product); //
 * Devuelve el producto creado en formato DTO return
 * ResponseEntity.status(HttpStatus.CREATED).body(ProductConvertTo.convertToDTO(
 * savedProduct)); }
 * 
 * // Mapeo de la ruta PUT para actualizar un producto por ID
 * 
 * @Operation(summary = "Update product by ID", description =
 ...
 * = @Schema(implementation = ErrorDetails.class))) })
 * 
 * @PutMapping("/products/{id}") public ResponseEntity<ProductDTO>
 * updateProduct(@PathVariable Long id, @RequestBody ProductDTO
 * updatedProductDTO) { // Busca el producto por ID en la base de datos
 * Optional<Product> productOptional = productRepository.findById(id); Product
 * product = productOptional // Si no se encuentra el producto, lanza una
 * excepción .orElseThrow(() -> new
 * ResourceNotFoundException("Product not found with id: " + id)); // Actualiza
 * componentes de la entidad product.setName(updatedProductDTO.getName());
 * product.setBrand(updatedProductDTO.getBrand());
 * product.setPrice(updatedProductDTO.getPrice()); // Guarda el producto
 * actualizado en la base de datos Product savedProduct =
 * productRepository.save(product); // Devuelve el producto actualizado en
 * formato DTO return
 * ResponseEntity.ok(ProductConvertTo.convertToDTO(savedProduct)); }
 * 
 ...
 * = @Schema(implementation = ErrorDetails.class))) })
 * 
 * @DeleteMapping("/products/{id}") public ResponseEntity<Void>
 * deleteProduct(@PathVariable Long id) { // Si el producto existe en la base de
 * datos if (productRepository.existsById(id)) { // Elimina el producto de la
 * base de datos productRepository.deleteById(id); // Devuelve contenido vacio
 * return ResponseEntity.noContent().build(); } else { throw new
