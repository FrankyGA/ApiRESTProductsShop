...
public class ClientController {
    ...
    public ResponseEntity<ClientDTO> updateClient(@PathVariable Long id, @RequestBody ClientDTO updatedClientDTO) {
        // Convierte el DTO del cliente en una entidad de cliente
        ...
        // Devuelve el cliente actualizado en formato DTO
 ...
 * // -------------- Petición todos los clientes --------------//
 * 
 * @Operation(summary = "Get all clients", description =
 * "Get a list of all clients")
 * 
 * @ApiResponses(value = {
 * 
 * @ApiResponse(responseCode = "200", description =
 * "Clients found, retrieved clients", content = {
 * 
 * @Content(mediaType = "application/json", array = @ArraySchema(schema
 * = @Schema(implementation = Client.class))) }),
 * 
 * @ApiResponse(responseCode = "404", description = "Clients not found", content
 * = @Content(schema = @Schema(implementation = ErrorDetails.class))) })
 * 
 * @GetMapping("/clients") public ResponseEntity<List<ClientDTO>>
 * getAllClients() { List<Client> clients = clientRepository.findAll(); if
 * (clients.isEmpty()) { throw new
 * ResourceNotFoundException("No clients found"); } return
 * ResponseEntity.ok(ClientConvertTo.convertToDTOList(clients)); }
 * 
 * // -------------- Petición cliente por ID --------------//
 * 
 * @Operation(summary = "Get client by ID", description =
 ...
 * "Client not found with id: ...", content = @Content(schema
 * = @Schema(implementation = ErrorDetails.class))) })
 * 
 * @GetMapping("/clients/{id}") public ResponseEntity<ClientDTO>
 * getClientById(@PathVariable Long id) { Optional<Client> clientOptional =
 * clientRepository.findById(id); Client client = clientOptional .orElseThrow(()
 * -> new ResourceNotFoundException("Client not found with id: " + id)); return
 * ResponseEntity.ok(ClientConvertTo.convertToDTO(client)); }
 * 
 * // -------------- Métodos peticiones Insertar --------------//
 * 
 * // Petición para insertar nuevo cliente
 ...
 * "Invalid client data provided", content = @Content(schema
 * = @Schema(implementation = ErrorDetails.class))) })
 * 
 * @PostMapping("/clients") public ResponseEntity<ClientDTO>
 * createClient(@RequestBody ClientDTO clientDTO) { if (clientDTO == null) {
 * throw new ValidationException("Invalid client data provided"); } Client
 * client = ClientConvertTo.convertToEntity(clientDTO); Client savedClient =
 * clientRepository.save(client); return
 * ResponseEntity.status(HttpStatus.CREATED).body(ClientConvertTo.convertToDTO(
 * savedClient)); }
 * 
 * // -------------- Métodos peticiones Modificar --------------//
 * 
 ...
 * = @Schema(implementation = ErrorDetails.class))) })
 * 
 * @PutMapping("/clients/{id}") public ResponseEntity<ClientDTO>
 * updateClient(@PathVariable Long id, @RequestBody ClientDTO updatedClientDTO)
 * { // Busca cliente por id para guardarlo en un objeto cliente
 * Optional<Client> clientOptional = clientRepository.findById(id); Client
 * client = clientOptional .orElseThrow(() -> new
 * ResourceNotFoundException("Client not found with id: " + id)); // Guarda los
 * atributos que se pasan en el cuerpo de la petición
 * client.setName(updatedClientDTO.getName());
 * client.setAddress(updatedClientDTO.getAddress());
 * client.setAge(updatedClientDTO.getAge()); // Guarda el cliente modificado y y
 * convierte en DTO para enviar los datos a la // respuesta Client savedClient =
 * clientRepository.save(client); return
 * ResponseEntity.ok(ClientConvertTo.convertToDTO(savedClient)); }
 * 
 * // -------------- Petición modificación de atributo cliente --------------//
 * 
 * // Método para actualizar solo el nombre de un cliente
 * 
 * @Operation(summary = "Update client name by ID", description =
 * "Update the name of an existing client by its ID")
 * 
 * @ApiResponses(value = {
 * 
 * @ApiResponse(responseCode = "200", description = "Client name updated"),
 ...
 * = @Schema(implementation = ErrorDetails.class))) })
 * 
 * @PatchMapping("/clients/{id}/updateName") public ResponseEntity<ClientDTO>
 * updateClientName(@PathVariable Long id, @RequestBody String newName) {
 * Optional<Client> clientOptional = clientRepository.findById(id); Client
 * client = clientOptional .orElseThrow(() -> new
 * ResourceNotFoundException("Client not found with id: " + id)); // Actualizar
 * solo el nombre del cliente client.setName(newName); // Guardar el cliente
 * actualizado en la base de datos Client savedClient =
 * clientRepository.save(client); // Retornar el cliente actualizado en formato
 * DTO return ResponseEntity.ok(ClientConvertTo.convertToDTO(savedClient)); }
 * 
 * // -------------- Métodos peticiones Borrar --------------//
 * 
 ...
 * clientRepository.deleteById(id); return ResponseEntity.noContent().build(); }
 * else { throw new ResourceNotFoundException("Client not found with id: " +
 * id); } } }
 */
